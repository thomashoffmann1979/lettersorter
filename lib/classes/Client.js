...
Client.prototype.ping = function(){
  ...
  for(tag in me._baos){
    if (me._baos.hasOwnProperty(tag)){
      list.push({tag: tag,filter: me._baos[tag].filter});
    }
  }
...
Client.prototype.start = function(){
  ...
  setInterval(me.ping.bind(me),5000);

  try{
    var magellan = new Magellan();
    magellan.read(function(inp){
  ...
  setTimeout(function(){
    me.lastSetup();
  },10000); // reading the last setup after n seconds
...
Client.prototype.lastSetup = function(){
  ...
  log('debug','Client','checking for '+path.join(osenv.home(),'.sorter_last_setup.json'));
  fs.exists(path.join(osenv.home(),'.sorter_last_setup.json'),function(exists){
    if (exists){
      try{
        var list = require(path.join(osenv.home(),'.sorter_last_setup.json'));
        for(var i=0;i<list.length;i++){
          if(list[i].tag){
            if (list[i].filter){
              if (me._baos[list[i].tag]){
                me.onInput(list[i].tag);
                me.onInput(list[i].filter);
              }
            }
          }
        }
...
Client.prototype.saveLastSetup = function(){
  var me=this,
      list = [];
  ;
  for(tag in me._baos){
  ...
  console.log(list);
  fs.writeFile(path.join(osenv.home(),'.sorter_last_setup.json'),JSON.stringify(list,null,4),function(err){
    if (err){
...
Client.prototype.onInput = function(data){
  ...
  log('debug','client','input '+input+' '+me.state+' #'+me._baosIndex);
...
Client.prototype.onFilterRemoved = function(o){
  ...
  log('info','client','filter removed by server '+tag);
  try{
    me._baos[tag].filter = '';
    me.freeWaitFor(tag);
  }catch(e){
