...
Master.prototype.filter = function(socket,filter){
  var me = this,
      i,
      m,
      r,
  ...
  if (typeof me._clientsByFilter[ filter ]==='object'){
    me._clientsByFilter[ filter ].socket.emit('filter removed',filter);
  ...
  if (client){
    me._clientsByFilter[filter] = client;
...
Master.prototype.addSending = function(sending){
  ...
  if (typeof me._clientsByFilter[filter]==='object'){
    me._clientsByFilter[filter].socket.emit('add id',sending.id);
  }
