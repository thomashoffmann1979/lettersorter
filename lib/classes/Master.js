...
Master.prototype.emitUI = function(name,msg){
  var me = this;
  if(typeof me._ui_client==='object'){
    me._ui_client.emit( name, msg);
  }
}
...
Master.prototype.removing = function(socket,v,callback){
  var me = this,
      i,
      m,
      r,
...
Master.prototype.erp = function(){
  ...
  me._erp.on('fastaccesstour',function(data){
    var i=0,m=data.length;
    for(i=0;i<m;i++){
      me.addFastaccess(data[i]);
    }
  })
...
Master.prototype.addFastaccess = function(item){
...
Master.prototype.addSending = function(sending){
  ...
  me._sendingsBySGSF['SGSF-'+sending.sg+'-'+sending.sf][sending.id]=0;
  ...
  if (typeof me._clientsByFilter[filter]==='object'){
    me._clientsByFilter[filter].client.socket.emit('add id',{
      data: sending.id,
      tag: me._clientsByFilter[filter].tag
    });
  }
...
//  console.log(filter,typeof me._clientsByFilter[filter]);
...
Master.prototype.initSocketEvents = function(socket){
  ...
  socket.on('new', me.onNew(socket).bind(me));
  socket.on('ui', me.onUI(socket).bind(me));
  socket.on('ocrservice', me.onOCR(socket).bind(me));
  socket.on('ping', me.onPing(socket).bind(me));
}


Master.prototype.onUI = function(socket){
  return function(data){
    var me = this;
    me._ui_client = socket;
    log('debug','master','added ui client '+JSON.stringify(data,null,0));
  }
}

Master.prototype.onOCR = function(socket){
  return function(data){
    var me = this;
    me._ocr_client = socket;
    me._ocr_client.emit('start',true)
    log('debug','master','added ocr client '+JSON.stringify(data,null,0));
  }
}


Master.prototype.onPing = function(socket){
  return function(data){
    var me = this;
    if (me._ui_client){
      me._ui_client.emit('boxPing',data);
    }
    log('debug','master','client ping '+JSON.stringify(data,null,0));
  }
}

Master.prototype.onNew = function(socket){
  return function(data){
    var me = this;

    var sending={
      id: data.codes[0],
      sg: data.sortresult.sortiergang,
      sf: data.sortresult.sortierfach,
      plz: data.sortresult.plz
    };
    me.addSending(sending);

    me._erp.put(sending,data.item);

    log('debug','master','new event '+JSON.stringify(sending,null,0));
  }
}
