...
PIN.prototype.start = function(){
  var me = this;
  fs.exists(me.currentPath,function(exists){
    if (exists){
      me.emit('started',true);
      log('debug','pin','exists '+me.currentPath);
    }else{
      fs.writeFile(path.join(gpioPath,'export'),me.pin,function(err){
        if (err){
          ...
          me.emit('started',true);
          log('debug','pin','export '+me.currentPath);
        }
      });
    }
  });
...
PIN.prototype.out = function(){
  var me = this;
  fs.writeFile(path.join(me.currentPath,'direction'),"out",function(err){
    if (err){
      me.emit('error',err);
...
PIN.prototype.in = function(controll){
  var me = this;
  fs.writeFile(path.join(me.currentPath,'direction'),"in",function(err){
    if (err){
      me.emit('error',err);
...
PIN.prototype.set = function(v){
  ...
  fs.writeFileSync(path.join(this.current,'value'),v);
}

PIN.prototype.get = function(v){
  return (fs.readFileSync(path.join(this.current,'value')).toString().trim() === '0')? false:true;
}
