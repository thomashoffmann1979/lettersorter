(function() {
  var BAO, Client, EventEmitter, LED, Magellan, PIFace, PIN, colors, fs, os, osenv, path, socket, udpfindme, util,
  ...
  module.exports = Client = (function(superClass) {
    ...
    Client.prototype._clean = function() {
      if (this.usePiFaceDigital === true) {
        this.baos['K-1'].PI.set(0, 0);
        this.baos['K-1'].PI.set(0, 1);
        this.baos['K-1'].PI.set(0, 2);
        this.baos['K-1'].PI.set(0, 3);
        this.baos['K-1'].PI.set(0, 4);
        this.baos['K-1'].PI.set(0, 5);
        this.baos['K-1'].PI.set(0, 6);
        this.baos['K-1'].PI.set(0, 7);
        return process.exit();
      }
    };
    ...
    Client.prototype.setUpPIFace = function(tag, delay, timeout, boardPin, optoPin, closeOnHiLO, motorPin) {
      this.usePiFaceDigital = true;
      this.baos[tag] = new PIFace(tag, delay, timeout, boardPin, optoPin, closeOnHiLO, motorPin);
      return this.baosIndex++;
    };
    ...
    Client.prototype.displayPinSetup = function() {
      var cl, endL, i, j, k, l, m, n, notes, o, p, pn, q, r, r0, r1, s, tag, v, w, x, y;
      endL = "\n";
      if (this.usePiFaceDigital === true) {
        w = 3;
        l = '';
        r0 = '';
        r1 = '';
        if (this.baos['K-1'].boardPin * 1 === 0) {
          r0 = 'B';
        }
        if (this.baos['K-1'].motorPin * 1 === 0) {
          r0 = 'M';
        }
        if (this.baos['K-1'].boardPin * 1 === 1) {
          r1 = 'B';
        }
        if (this.baos['K-1'].motorPin * 1 === 1) {
          r1 = 'M';
        }
        l += colors.black('----') + endL;
        ...
        for (i = m = 0; m <= 9; i = ++m) {
          v = '*';
          if (this.baos['K-1'].boardPin * 1 === i) {
            v = 'B';
          }
          if (this.baos['K-1'].motorPin * 1 === i) {
            v = 'M';
          }
          l += colors.yellow('|' + v);
        }
