(function() {
  var EventEmitter, Master, socketio,
  ...
  module.exports = Master = (function(superClass) {
    ...
    Master.prototype.start = function() {
      ...
      this.erp.on('logged in', (function(_this) {
        return function(sid) {
          return _this.onERPLogin(sid);
        };
      })(this));
      this.erp.on('error', (function(_this) {
        return function(error) {
          return _this.onERPError(error);
        };
      })(this));
      this.erp.on('sendings', (function(_this) {
        return function(sendings) {
          return _this.onERPSendings(sendings);
        };
      })(this));
      return this.erp.login();
    };

    Master.prototype.onERPLogin = function(sid) {
      this.emit('logged in', sid);
      return this.erp.sendings();
    };

    Master.prototype.onERPError = function(error) {
      return this.emit('error', error);
    };

    Master.prototype.onERPSendings = function(sendings) {
      var i, item, len, results;
      results = [];
      for (i = 0, len = sendings.length; i < len; i++) {
        item = sendings[i];
    ...
    Master.prototype.onIncommingConnection = function(socket) {
      ...
      return socket.on('ocrservice', (function(_this) {
        return function(data) {
