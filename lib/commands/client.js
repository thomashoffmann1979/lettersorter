  ...
  params = [
    {
      ...
      parameter: "--pifacedigital",
      description: "use pifacedigital"
    }, {
      parameter: "--board [board]",
      description: "board pin number (only pifacedigital)"
    }, {
      parameter: "--opto [opto]",
      description: "opto pin number (only pifacedigital)"
    }, {
      parameter: "--motor [motor]",
      description: "motor pin number (only pifacedigital)"
    }, {
      parameter: "-l, --lohi",
  ...
  module.exports = ClientCMD = (function(superClass) {
    ...
    ClientCMD.prototype.action = function(options, args) {
      ...
      if (options.pifacedigital) {
        tag = 'K-1';
        if (options.motor) {
          this.client.setUpPIFace(tag, gd, gt, options.board, options.opto, closeOnHiLO, options.motor);
        } else {
          this.client.setUpPIFace(tag, gd, gt, options.board, options.opto, closeOnHiLO);
        }
